...
# Assuming your data frame is named 'neo_data'
boxplot(diameter ~ neo_type, data = df_neo_dia, col = "lightblue", main = "Asteroid Diameters by NEO Type")
...
df_neo_dia %>%
  ...
  ggplot(aes(x = neo_type, y = avg_diameter, fill = neo_type)) +
  geom_bar(stat = "identity") +
  labs(title = "Average Asteroid Diameter by NEO Type",
       x = "NEO Type",
       y = "Average Diameter")
...
# Assuming your data frame is named 'df_neo_dia'
# Make sure 'diameter' and 'neo_type' are the correct column names

# Perform ANOVA
anova_result <- aov(diameter ~ neo_type, data = df_neo_dia)
summary(anova_result)

```
Given significance level of 0.0001, the p-value (0.00448) is still less than 0.0001. Therefore, reject the null hypothesis.

```{r EDA Diameter_Difference Pair t-test, echo=T}
pairwise.t.test(df_neo_dia$diameter, df_neo_dia$neo_type, p.adjust.method = "bonferroni")
```


### Verdict

The results of the ANOVA test suggest that there are statistically significant differences in average diameters among the different NEO types (Amors, Apollos, Atens, Atiras). 
